...
async def test_plan_returns_sub_questions_under_cap() -> None:
    ...
    result = await plan("big question", provider=provider, cap=5)
...
async def test_plan_strips_blank_sub_questions() -> None:
    ...
    result = await plan("q", provider=provider, cap=5)
    assert result == ["q1", "q2"]
...
async def test_plan_raises_on_empty() -> None:
    provider = FakeLLMProvider(responses=[_plan_response()])
    with pytest.raises(PlannerError):
        await plan("q", provider=provider)

// ============================================================
//  NEXUS — simulated research engine
//  Mirrors the real backend contract (handoff §3): a research job
//  emits AgentEvents while `running`, then resolves to
//  { report (markdown w/ [n]), sources, consulted, gaps }.
//  Until the streaming API lands, the UI polls; here we script it.
// ============================================================
(function () {
  // ----- The flagship demo run (loaded by the example chips) -----
  const GRID = {
...
**Bottom line:** pair LFP (and increasingly sodium-ion) for daily cycling with an emerging LDES layer — flow and iron-air leading — and let policy plus arbitrage economics decide the mix region by region [1][3][4].`,
  ...
  // A second canned example for variety.
  const LLM = {
    ...
    researchers: [
      { q: "Capability of small models", tools: [
      ...
      ], found: 1, gap: "Independent battery-draw measurements for sustained on-device inference were not available from a primary source." },
      { q: "Trade-offs vs cloud", tools: [
...
**Bottom line:** small on-device models won't replace frontier systems, but they're absorbing the long tail of everyday AI — privately, instantly, and essentially for free [1][3].`,
  ...
  const PRESETS = [GRID, LLM];

  // ----- Generic builder for an arbitrary user prompt -----
  function buildGeneric(prompt) {
    ...
    return {
      ...
      researchers: [
        { q: "Background & definitions", tools: [
...
> This is a **simulated** run wired to the live agent-feed UI. Connect the Nexus API and the same interface streams real plans, searches, and a cited report.`,
  ...
  // ----- Flatten a run into an ordered event timeline the UI reveals -----
  // Each event: { id, kind, delay, ... }. delay = ms to wait before showing.
  function toTimeline(run) {
    const ev = [];
    let id = 0;
    const push = (e, delay) => ev.push(Object.assign({ id: id++, delay }, e));

    push({ kind: "planner", state: "start", title: "Planning your research", sub: "Decomposing the question into sub-questions to research in parallel." }, 500);
    push({ kind: "plan", items: run.plan }, 1300);

    run.researchers.forEach((r, i) => {
      push({ kind: "researcher", state: "start", index: i + 1, total: run.researchers.length, question: r.q }, 900);
      r.tools.forEach((tool) => {
        push({ kind: "tool", action: tool.action, text: tool.text, domain: tool.domain, title: tool.title }, tool.action === "read" ? 850 : 700);
      });
      const sub = r.gap
    ...
    push({ kind: "writer", state: "start", title: "Writing the report", sub: `Synthesizing ${run.researchers.length} findings into a single cited report.` }, 1000);
    push({ kind: "writer", state: "done", title: "Report ready", sub: "Citations linked to sources." }, 1600);

    // collect gaps from researchers
    const gaps = run.researchers.filter(r => r.gap).map(r => r.gap);
    const result = Object.assign({}, run.result, { gaps: gaps.concat(run.result.gaps || []) });
    return { timeline: ev, result };
  }

  window.NexusData = { PRESETS, pickRun, toTimeline, buildGeneric };
})();
